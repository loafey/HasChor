...
type Choreo m = Network m --Freer (ChoreoSig m)
...
locally :: KnownSymbol l
        ...
        -> ((forall a . a @ l -> a) -> m a) -- ^ The local computation given a constrained
                             -- unwrap funciton.
        -> Choreo m (a @ l)
--locally l m = toFreer (Local l m)
locally _ _ = error $ concat ["can not invoke locally with polymorphic locations, location must be explicit. Inline, perhaps?"]
...
(~>) (l, a) l' = error $ concat ["can not communicate when either the sender or receiver is location polymorphic, must be explicit. Inline, perhaps?"]
...
cond (l, a) c = error $ concat ["can not use cond in a location polymorphic manner, must have explicit location. Inline, perhaps?"]
...
--   x <- l `locally` m
--   (l, x) ~> l'
(~~>) (l, m) l' = error $ concat ["locallysend rewrite rule did not fire, debug it"]
...
--   x <- l `locally` m
--   cond (l, x) c
cond' (l, m) c = error $ concat ["condprime rewrite rule did not fire, debug it"]
